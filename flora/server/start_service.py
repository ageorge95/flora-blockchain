...
from flora.daemon.server import service_launch_lock_path
from flora.rpc.rpc_server import RpcApiProtocol, RpcServer, RpcServiceProtocol, start_rpc_server
from flora.server.flora_policy import set_flora_policy
...
from flora.types.peer_info import PeerInfo, UnresolvedPeerInfo
...
class Service(Generic[_T_RpcServiceProtocol]):
    def __init__(
        ...
        *,
        ...
        on_connect_callback: Optional[Callable[[WSFloraConnection], Awaitable[None]]] = None,
        ...
        self._log.info(f"flora-blockchain version: {flora_full_version_str()}")
        ...
        self._server = FloraServer.create(
            advertised_port,
            node,
            peer_api,
            node_type,
    ...
    async def setup_process_global_state(self) -> None:
        ...
        proctitle_name = f"flora_{self._service_name}"
...
def async_run(coro: Coroutine[object, object, T], connection_limit: Optional[int] = None) -> T:
    if connection_limit is not None:
        set_flora_policy(connection_limit)
