...
from flora.protocols.protocol_state_machine import message_requires_reply
from flora.protocols.protocol_timing import INVALID_PROTOCOL_BAN_SECONDS
...
from flora.server.ws_connection import ConnectionCallback, WSFloraConnection
...
from flora.util.network import WebServer, is_in_network, is_localhost, is_trusted_peer
from flora.util.ssl_check import verify_ssl_certs_and_keys
...
class FloraServer:
    ...
    all_connections: Dict[bytes32, WSFloraConnection] = field(default_factory=dict)
    ...
    def create(
        ...
        flora_ca_crt_path, flora_ca_key_path = flora_ca_crt_key
        ...
        if local_type in authenticated_client_types:
            ...
            ssl_client_context = ssl_context_for_client(
                ca_cert=flora_ca_crt_path,
                ca_key=flora_ca_key_path,
    ...
    async def start(
        ...
        if self.webserver is not None:
            raise RuntimeError("FloraServer already started")
    ...
    async def incoming_connection(self, request: web.Request) -> web.StreamResponse:
        ...
        try:
            connection = WSFloraConnection.create(
    ...
    async def connection_added(
        self, connection: WSFloraConnection, on_connect: Optional[ConnectionCallback] = None
    ) -> None:
    ...
    def connection_closed(self, connection: WSFloraConnection, ban_time: int, closed_connection: bool = False) -> None:
    ...
    async def get_peer_info(self) -> Optional[PeerInfo]:
        ...
        # Use flora's service first.
        try:
            timeout = ClientTimeout(total=15)
            async with ClientSession(timeout=timeout) as session:
                async with session.get("https://ip.flora.net/") as resp:
                    if resp.status == 200:
    ...
    def is_trusted_peer(self, peer: WSFloraConnection, trusted_peers: Dict[str, Any]) -> bool:
