...
from flora.util.ssl_check import DEFAULT_PERMISSIONS_CERT_FILE, DEFAULT_PERMISSIONS_KEY_FILE
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("floracoin.farm")]),
            critical=False,
        )
...
def create_all_ssl(
    ...
    *,
    ...
    write_ssl_cert_and_key(flora_ca_crt_path, flora_ca_crt, flora_ca_key_path, flora_ca_key, overwrite=overwrite)
