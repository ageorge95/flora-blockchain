...
from flora.data_layer.data_layer_errors import OfferIntegrityError
from flora.data_layer.data_layer_util import OfferStore, ProofOfInclusion, ProofOfInclusionLayer, StoreProofs, leaf_hash
...
from flora.types.blockchain_format.serialized_program import SerializedProgram
...
from flora.wallet.db_wallet.db_wallet_puzzles import (
    ACS_MU,
    ACS_MU_PH,
    GRAFTROOT_DL_OFFERS,
    SINGLETON_LAUNCHER,
...
from flora.wallet.puzzles.singleton_top_layer_v1_1 import SINGLETON_LAUNCHER_HASH
from flora.wallet.sign_coin_spends import sign_coin_spends
from flora.wallet.trading.offer import NotarizedPayment, Offer
from flora.wallet.transaction_record import TransactionRecord
from flora.wallet.util.compute_memos import compute_memos
from flora.wallet.util.merkle_utils import _simplify_merkle_proof
from flora.wallet.util.transaction_type import TransactionType
from flora.wallet.util.wallet_sync_utils import fetch_coin_spend, fetch_coin_spend_for_coin_state
...
class DataLayerWallet:
    ...
    async def get_launcher_coin_state(self, launcher_id: bytes32, peer: WSFloraConnection) -> CoinState:
    ...
    async def create_tandem_xfl_tx(
        self,
        fee: uint64,
        announcement_to_assert: Announcement,
        coin_announcement: bool = True,
    ) -> TransactionRecord:
        flora_tx = await self.standard_wallet.generate_signed_transaction(
            amount=uint64(0),
        ...
        assert flora_tx.spend_bundle is not None
        return flora_tx
    ...
    async def delete_mirror(
        self, mirror_id: bytes32, peer: WSFloraConnection, fee: uint64 = uint64(0)
    ) -> List[TransactionRecord]:
        ...
        if excess_fee > 0:
            flora_tx: TransactionRecord = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
                uint64(1),
                new_puzhash,
            ...
            txs = [
                dataclasses.replace(
                    txs[0], spend_bundle=SpendBundle.aggregate([txs[0].spend_bundle, flora_tx.spend_bundle])
                ),
                dataclasses.replace(flora_tx, spend_bundle=None),
            ]
