...
from flora.daemon.server import WebSocketServer, daemon_launch_lock_path
from flora.simulator.full_node_simulator import FullNodeSimulator
from flora.simulator.socket import find_available_listen_port
from flora.simulator.ssl_certs import (
...
from flora.simulator.start_simulator import async_main as start_simulator_main
from flora.ssl.create_ssl import create_all_ssl
from flora.types.blockchain_format.sized_bytes import bytes32
from flora.util.bech32m import encode_puzzle_hash
from flora.util.config import create_default_flora_config, load_config, save_config
from flora.util.errors import KeychainFingerprintExists
from flora.util.ints import uint32
from flora.util.keychain import Keychain
from flora.util.lock import Lockfile
...
def create_config(
    ...
    # create flora directories
    create_default_flora_config(flora_root)
    create_all_ssl(
        flora_root,
    ...
    # load config
    config = load_config(flora_root, "config.yaml")
    ...
    config["simulator"]["farming_address"] = encode_puzzle_hash(get_puzzle_hash_from_key(keychain, fingerprint), "txfl")
    config["simulator"]["plot_directory"] = "test-simulator/plots"
    # save config
    save_config(flora_root, "config.yaml", config)
    return config


async def start_simulator(flora_root: Path, automated_testing: bool = False) -> AsyncGenerator[FullNodeSimulator, None]:
    sys.argv = [sys.argv[0]]  # clear sys.argv to avoid issues with config.yaml
    service = await start_simulator_main(True, automated_testing, root_path=flora_root)
    await service.start()
...
async def get_full_flora_simulator(
    flora_root: Path,
    ...
    """
    A flora root Path is required.
    The flora root Path can be a temporary directory (tempfile.TemporaryDirectory)
    ...
    This test is fully interdependent and can be used without the rest of the flora test suite.
    ...
    with Lockfile.create(daemon_launch_lock_path(flora_root)):
        ...
        crt_path = flora_root / config["daemon_ssl"]["private_crt"]
        key_path = flora_root / config["daemon_ssl"]["private_key"]
        ca_crt_path = flora_root / config["private_ssl_ca"]["crt"]
        ca_key_path = flora_root / config["private_ssl_ca"]["key"]

        ws_server = WebSocketServer(flora_root, ca_crt_path, ca_key_path, crt_path, key_path)
        await ws_server.setup_process_global_state()
        async with ws_server.run():
            async for simulator in start_simulator(flora_root, automated_testing):
                yield simulator, flora_root, config, mnemonic, fingerprint, keychain
