...
from flora.cmds.init_funcs import init
...
from flora.protocols.shared_protocol import Capability, capabilities
from flora.server.start_farmer import create_farmer_service
from flora.server.start_full_node import create_full_node_service
from flora.server.start_harvester import create_harvester_service
from flora.server.start_introducer import create_introducer_service
from flora.server.start_service import Service
from flora.server.start_timelord import create_timelord_service
from flora.server.start_wallet import create_wallet_service
from flora.simulator.block_tools import BlockTools
from flora.simulator.keyring import TempKeyring
from flora.simulator.start_simulator import create_full_node_simulator_service
from flora.timelord.timelord import Timelord
from flora.timelord.timelord_launcher import kill_processes, spawn_process
from flora.types.peer_info import UnresolvedPeerInfo
from flora.util.bech32m import encode_puzzle_hash
from flora.util.config import config_path_for_filename, lock_and_load_config, save_config
from flora.util.ints import uint16
from flora.util.keychain import bytes_to_mnemonic
...
async def setup_wallet_node(
    ...
    with TempKeyring(populate=True) as keychain:
        ...
        service_config["xfl_spam_amount"] = xfl_spam_amount
...
async def setup_farmer(
    ...
    service_config["xfl_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xfl")
    service_config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    service_config["port"] = port
    service_config["rpc_port"] = uint16(0)
    config_pool["xfl_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xfl")
