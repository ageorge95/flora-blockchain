...
from flora.plotters.plotters_util import get_venv_bin, reset_loop_policy_for_windows, run_command, run_plotter
from flora.plotting.create_plots import resolve_plot_keys
...
def get_madmax_exec_venv_path(ksize: int = 32) -> Optional[Path]:
    ...
    if not venv_bin_path:
        return None
    madmax_exec = "flora_plot"
    if ksize > 32:
        madmax_exec += "_k34"  # Use the flora_plot_k34 executable for k-sizes > 32
...
def plot_madmax(args, flora_root_path: Path, plotters_root_path: Path):
    ...
    try:
        asyncio.run(run_plotter(flora_root_path, args.plotter, call_args, progress))
    except Exception as e:
