...
# https://github.com/Flora-Network/chips/blob/80e4611fe52b174bf1a0382b9dff73805b18b8c6/CHIPs/chip-0002.md#signmessage
CHIP_0002_SIGN_MESSAGE_PREFIX = "Flora Signed Message"
...
class Wallet:
    ...
    async def create_tandem_xfl_tx(
        ...
        flora_coins = await self.select_coins(fee)
        if reuse_puzhash is None:
            reuse_puzhash_config = self.wallet_state_manager.config.get("reuse_public_key_for_change", None)
            if reuse_puzhash_config is None:
                reuse_puzhash = False
        ...
        flora_tx = await self.generate_signed_transaction(
            uint64(0),
    ...
    # WSFloraConnection is only imported for type checking
    async def coin_added(
        self, coin: Coin, height: uint32, peer: WSFloraConnection
