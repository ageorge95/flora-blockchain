...
from flora.consensus.coinbase import farmer_parent_id, pool_parent_id
from flora.consensus.constants import ConsensusConstants
from flora.data_layer.data_layer_wallet import DataLayerWallet
from flora.data_layer.dl_wallet_store import DataLayerStore
from flora.pools.pool_puzzles import SINGLETON_LAUNCHER_HASH, solution_to_pool_state
...
from flora.util.db_wrapper import DBWrapper2
...
from flora.wallet.cat_wallet.cat_utils import construct_cat_puzzle, match_cat_puzzle
from flora.wallet.cat_wallet.cat_wallet import CATWallet
from flora.wallet.db_wallet.db_wallet_puzzles import MIRROR_PUZZLE_HASH
...
from flora.wallet.did_wallet.did_wallet_puzzles import DID_INNERPUZ_MOD, match_did_puzzle
from flora.wallet.key_val_store import KeyValStore
from flora.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs, get_new_owner_did
from flora.wallet.nft_wallet.nft_wallet import NFTWallet
from flora.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from flora.wallet.notification_manager import NotificationManager
from flora.wallet.outer_puzzles import AssetType
from flora.wallet.puzzle_drivers import PuzzleInfo
from flora.wallet.puzzles.cat_loader import CAT_MOD, CAT_MOD_HASH
...
from flora.wallet.util.address_type import AddressType
...
from flora.wallet.util.wallet_types import WalletIdentifier, WalletType
...
from flora.wallet.wallet_retry_store import WalletRetryStore
...
class WalletStateManager:
    ...
    async def determine_coin_type(
        self, peer: WSFloraConnection, coin_state: CoinState, fork_height: Optional[uint32]
    ...
    async def filter_spam(self, new_coin_state: List[CoinState]) -> List[CoinState]:
        xfl_spam_amount = self.config.get("xfl_spam_amount", 1000000)

        # No need to filter anything if the filter is set to 1 or 0 mojos
        if xfl_spam_amount <= 1:
            return new_coin_state

        spam_filter_after_n_txs = self.config.get("spam_filter_after_n_txs", 200)
        small_unspent_count = await self.coin_store.count_small_unspent(xfl_spam_amount)
        ...
        for cs in new_coin_state:
            ...
            if (
                ...
                and cs.coin.amount < xfl_spam_amount
    ...
    async def get_minter_did(self, launcher_coin: Coin, peer: WSFloraConnection) -> Optional[bytes32]:
        # Get minter DID
