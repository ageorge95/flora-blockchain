...
from flora.wallet.notification_store import Notification, NotificationStore
from flora.wallet.transaction_record import TransactionRecord
from flora.wallet.util.compute_memos import compute_memos_for_spend
from flora.wallet.util.notifications import construct_notification
...
class NotificationManager:
    ...
    async def send_new_notification(
        ...
        flora_tx = await self.wallet_state_manager.main_wallet.generate_signed_transaction(
            amount,
            notification_hash,
            fee,
            coins=coins,
        ...
        full_tx: TransactionRecord = dataclasses.replace(
            flora_tx, spend_bundle=SpendBundle.aggregate([flora_tx.spend_bundle, extra_spend_bundle])
        )
        return full_tx
