...
from flora.wallet.cat_wallet.cat_info import CATInfo, LegacyCATInfo
...
from flora.wallet.puzzles.tails import ALL_LIMITATIONS_PROGRAMS
...
class CATWallet:
    ...
    async def create_new_cat_wallet(
        ...
        try:
            flora_tx, spend_bundle = await ALL_LIMITATIONS_PROGRAMS[
        ...
        flora_tx = dataclasses.replace(flora_tx, spend_bundle=spend_bundle)
        await self.standard_wallet.push_transaction(flora_tx)
    ...
    async def create_tandem_xfl_tx(
        ...
        if fee > amount_to_claim:
            flora_coins = await self.standard_wallet.select_coins(
                fee,
                min_coin_amount=min_coin_amount,
                max_coin_amount=max_coin_amount,
                excluded_coin_amounts=exclude_coin_amounts,
            )
            origin_id = list(flora_coins)[0].name()
            ...
            message = None
            for spend in flora_tx.spend_bundle.coin_spends:
            ...
            selected_amount = sum([c.amount for c in flora_coins])
            flora_tx = await self.standard_wallet.generate_signed_transaction(
    ...
    async def generate_unsigned_spendbundle(
        ...
        # Figure out if we need to absorb/melt some XFL as part of this
        regular_flora_to_claim: int = 0
        if payment_amount > starting_amount:
            fee = uint64(fee + payment_amount - starting_amount)
        elif payment_amount < starting_amount:
            regular_flora_to_claim = payment_amount

        need_flora_transaction = (fee > 0 or regular_flora_to_claim > 0) and (fee - regular_flora_to_claim != 0)
        ...
        for coin in cat_coins:
            if first:
                ...
                if need_flora_transaction:
                    if fee > regular_flora_to_claim:
                        flora_tx, _ = await self.create_tandem_xfl_tx(
                            fee,
                            uint64(regular_flora_to_claim),
                    ...
                    elif regular_flora_to_claim > fee:
                        flora_tx, _ = await self.create_tandem_xfl_tx(
                            fee,
                            uint64(regular_flora_to_claim),
                            min_coin_amount=min_coin_amount,
        ...
        flora_spend_bundle = SpendBundle([], G2Element())
        if flora_tx is not None and flora_tx.spend_bundle is not None:
            flora_spend_bundle = flora_tx.spend_bundle
    ...
    async def generate_signed_transaction(
        ...
        if flora_tx is not None:
            tx_list.append(
                TransactionRecord(
                    confirmed_at_height=flora_tx.confirmed_at_height,
                    created_at_time=flora_tx.created_at_time,
                    to_puzzle_hash=flora_tx.to_puzzle_hash,
                    amount=flora_tx.amount,
                    fee_amount=flora_tx.fee_amount,
                    confirmed=flora_tx.confirmed,
                    sent=flora_tx.sent,
                    spend_bundle=None,
                    additions=flora_tx.additions,
                    removals=flora_tx.removals,
                    wallet_id=flora_tx.wallet_id,
                    sent_to=flora_tx.sent_to,
                    trade_id=flora_tx.trade_id,
                    type=flora_tx.type,
                    name=flora_tx.name,
                    memos=[],
                )
