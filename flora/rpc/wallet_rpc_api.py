...
from flora.data_layer.data_layer_wallet import DataLayerWallet
from flora.pools.pool_wallet import PoolWallet
from flora.pools.pool_wallet_info import FARMING_TO_POOL, PoolState, PoolWalletInfo, create_pool_state
...
from flora.rpc.rpc_server import Endpoint, EndpointResult, default_get_connections
...
from flora.types.blockchain_format.coin import Coin, coin_as_list
...
from flora.types.coin_spend import CoinSpend
from flora.types.signing_mode import SigningMode
...
from flora.util.errors import KeychainIsLocked
from flora.util.ints import uint16, uint32, uint64
...
from flora.wallet.cat_wallet.cat_constants import DEFAULT_CATS
from flora.wallet.cat_wallet.cat_wallet import CATWallet
...
from flora.wallet.did_wallet.did_wallet_puzzles import (
    DID_INNERPUZ_MOD,
    match_did_puzzle,
    metadata_to_program,
    program_to_metadata,
...
from flora.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTInfo
from flora.wallet.nft_wallet.nft_puzzles import get_metadata_and_phs
from flora.wallet.nft_wallet.nft_wallet import NFTWallet
from flora.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from flora.wallet.notification_store import Notification
from flora.wallet.outer_puzzles import AssetType
from flora.wallet.puzzle_drivers import PuzzleInfo, Solver
from flora.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import puzzle_hash_for_synthetic_public_key
from flora.wallet.singleton import create_singleton_puzzle
from flora.wallet.trade_record import TradeRecord
from flora.wallet.trading.offer import Offer
from flora.wallet.transaction_record import TransactionRecord
from flora.wallet.uncurried_puzzle import uncurry_puzzle
from flora.wallet.util.address_type import AddressType, is_valid_address
from flora.wallet.util.compute_hints import compute_coin_hints
from flora.wallet.util.compute_memos import compute_memos
from flora.wallet.util.transaction_type import TransactionType
from flora.wallet.util.wallet_sync_utils import fetch_coin_spend_for_coin_state
...
class WalletRpcApi:
    ...
    async def get_latest_singleton_coin_spend(
        self, peer: WSFloraConnection, coin_id: bytes32, latest: bool = True
    ...
    async def _check_key_used_for_rewards(
        ...
        farmer_target = config["farmer"].get("xfl_target_address")
        pool_target = config["pool"].get("xfl_target_address")
    ...
    async def verify_signature(self, request) -> EndpointResult:
        ...
        # signatures made by `flora keys sign`, which uses BLS_MESSAGE_AUGMENTATION_HEX_INPUT
    ...
    async def get_offer_summary(self, request) -> EndpointResult:
        ...
        from flora.util.bech32m import bech32_decode, convertbits
        from flora.wallet.util.puzzle_compression import decompress_object_with_puzzles
    ...
    async def cancel_offers(self, request: Dict) -> EndpointResult:
        ...
        if cancel_all:
            ...
            asset_id = request.get("asset_id", "xfl")
        ...
        key = None
        if asset_id is not None and asset_id != "xfl":
    ...
    async def did_get_info(self, request) -> EndpointResult:
        ...
        return {
            ...
            "p2_address": encode_puzzle_hash(p2_puzzle.get_tree_hash(), AddressType.XFL.hrp(self.service.config)),
    ...
    async def nft_mint_bulk(self, request) -> EndpointResult:
        ...
        xfl_coin_list = request.get("xfl_coins", None)
        xfl_coins = None
        if xfl_coin_list:
            xfl_coins = set([Coin.from_json_dict(xfl_coin) for xfl_coin in xfl_coin_list])
        xfl_change_target = request.get("xfl_change_target", None)
        if xfl_change_target is not None:
            if xfl_change_target[:2] == "xfl":
                xfl_change_ph = decode_puzzle_hash(xfl_change_target)
            else:
                xfl_change_ph = bytes32(hexstr_to_bytes(xfl_change_target))
        else:
            xfl_change_ph = None
        ...
        if mint_from_did:
            ...
            sb = await nft_wallet.mint_from_xfl(
                metadata_list,
                mint_number_start=mint_number_start,
                mint_total=mint_total,
                target_list=target_list,
