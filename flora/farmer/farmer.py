...
from flora.plot_sync.delta import Delta
from flora.plot_sync.receiver import Receiver
from flora.pools.pool_config import PoolWalletConfig, add_auth_key, load_pool_config
...
from flora.server.server import FloraServer, ssl_context_for_root
...
from flora.util.config import config_path_for_filename, load_config, lock_and_load_config, save_config
from flora.util.errors import KeychainProxyConnectionFailure
from flora.util.hash import std_hash
from flora.util.ints import uint8, uint16, uint64
from flora.util.keychain import Keychain
from flora.util.logging import TimedDuplicateFilter
...
class Farmer:
    ...
    async def setup_keys(self) -> bool:
        no_keys_error_str = "No keys exist. Please run 'flora keys generate' or open the UI."
        try:
        ...
        if "xfl_target_address" not in self.config:
            self.config = config["farmer"]
        if "xfl_target_address" not in self.pool_config:
            self.pool_config = config["pool"]
        if "xfl_target_address" not in self.config or "xfl_target_address" not in self.pool_config:
            log.debug("xfl_target_address missing in the config")
            return False
