...
from flora.cmds.wallet_funcs import get_mojo_per_unit, get_wallet_type, print_balance
...
async def async_combine(args: Dict[str, Any], wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    final_fee = uint64(int(Decimal(args["fee"]) * units["flora"]))
    ...
    if not await wallet_client.get_synced():
        print("Wallet not synced. Please wait.")
        return
    is_xfl: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly combining Flora
    ...
    if final_target_coin_amount != 0:  # if we have a set target, just use standard coin selection.
        removals: List[Coin] = await wallet_client.select_coins(
            amount=(final_target_coin_amount + final_fee) if is_xfl else final_target_coin_amount,
            wallet_id=wallet_id,
    ...
    if is_xfl and total_amount - final_fee <= 0:
    ...
    additions = [{"amount": (total_amount - final_fee) if is_xfl else total_amount, "puzzle_hash": target_ph}]
...
async def async_split(args: Dict[str, Any], wallet_client: WalletRpcClient, fingerprint: int) -> None:
    ...
    if not await wallet_client.get_synced():
        print("Wallet not synced. Please wait.")
        return
    is_xfl: bool = wallet_type == WalletType.STANDARD_WALLET  # this lets us know if we are directly spitting Flora
