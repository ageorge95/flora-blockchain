...
from flora.cmds.configure import configure
    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xfl_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xfl_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xfl_target_address") or all_targets[-2] == config["pool"].get(
                    "xfl_target_address"
                ):
        ...
        if "xfl_target_address" not in config["farmer"]:
            print(
                f"Setting the xfl destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
        ...
        if "xfl_target_address" not in config["pool"]:
            print(f"Setting the xfl destination address for pool reward (7/8 for solo only) to {all_targets[0]}")
        ...
        if updated_target:
            print(
                f"To change the XFL destination addresses, edit the `xfl_target_address` entries in"
...
def init(
    ...
    if create_certs is not None:
        ...
        return flora_init(root_path, fix_ssl_permissions=fix_ssl_permissions, testnet=testnet, v1_db=v1_db)
...
def flora_init(
    ...
    *,
    ...
    """
    flora_root = os.environ.get("FLORA_ROOT", None)
    if flora_root is not None:
        print(f"FLORA_ROOT is set to {flora_root}")
