...
from flora.cmds.check_wallet_db import help_text as check_help_text
from flora.cmds.cmds_util import execute_with_wallet
from flora.cmds.coins import coins_cmd
from flora.cmds.plotnft import validate_fee
from flora.wallet.transaction_sorting import SortKey
...
@click.option(
    "-ma",
    "--min-coin-amount",
    help="Ignore coins worth less then this much XFL or CAT units",
    type=str,
    required=False,
    default="0",
)
@click.option(
    "-l",
    "--max-coin-amount",
    help="Ignore coins worth more then this much XFL or CAT units",
    type=str,
    required=False,
    default="0",
)
...
def address_sign_message(wallet_rpc_port: Optional[int], fingerprint: int, address: str, hex_message: str) -> None:
    extra_params: Dict[str, Any] = {"address": address, "message": hex_message, "type": AddressType.XFL}
    import asyncio
...
@wallet_cmd.command("make_offer", short_help="Create an offer of XFL/CATs/NFTs for XFL/CATs/NFTs")
@click.option(
    "-wp",
    "--wallet-rpc-port",
    help="Set the port where the Wallet is hosting the RPC interface. See the rpc_port under wallet in config.yaml",
...
@click.option(
    "-m", "--fee", help="A fee to add to the offer when it gets taken, in XFL", default="0", show_default=True
)
@click.option(
    "-r",
    "--reuse",
...
@click.option(
    "-m", "--fee", help="The fee to use when pushing the completed offer, in XFL", default="0", show_default=True
)
@click.option(
    "-r",
    "--reuse",
...
@click.option(
    "-m", "--fee", help="The fee to use when cancelling the offer securely, in XFL", default="0", show_default=True
)
...
def check_wallet_cmd(ctx: click.Context, db_path: str, verbose: bool) -> None:
    """check, scan, diagnose, fsck Flora Wallet DBs"""
    import asyncio

    from flora.cmds.check_wallet_db import scan
...
@click.option(
    "-m",
    "--fee",
    help="Set the fees per transaction, in XFL.",
    type=str,
    default="0",
    show_default=True,
